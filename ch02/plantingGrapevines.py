...
endPostSpace = float(
    input("Enter the amount of space used by an end-post assembly (in feet): ")
)
vineSpace = float(input("Enter the amount of space between the vines (in feet): "))

numGrapeVinesInRow = (length - (2 * endPostSpace)) / vineSpace

print(f"The number of grapevines that will fit in the row: {numGrapeVinesInRow:.2f}")
